...
T8430Query::T8430Query(QObject *parent) : TrQuery(tr(T8430_NAME), parent)
{
...
void T8430Query::onReceiveData(const QString &trCode)
{
    ...
    int nCount = xaquery()->GetBlockCount(T8430_OUT_BLOCK_NAME);
    for( int index = 0; index<nCount; index++){
        T8430Item *item = new T8430Item();
        foreach(QString property , item->getPropertyList()) {
            QString fieldValue = xaquery()->GetFieldData(T8430_OUT_BLOCK_NAME, property, index);
